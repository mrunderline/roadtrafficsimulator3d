...
TRAFFIC.Car.prototype = {
  ...
  getAcceleration: function () {
    var a, b, breakGap, busyRoadCoeff, coeff, deltaSpeed, distanceGap, distanceToNextCar, freeRoadCoeff,
      intersectionCoeff, nextCarDistance, safeDistance, safeIntersectionDistance, timeGap, _ref;
  ...
  move: function (delta) {
    ...
    const speed_limitation = this.trajectory.current.lane.road.max_speed;
    let considered_speed = this.speed;
  ...
  popNextLane: function () {
    ...
    Object.keys(map_data_js.specialCars).forEach(key => {
      if (map_data_js.specialCars[key].id === this.id) {
        return this.traveled_distance = this.nextLane.length;
      }
    });
    return nextLane;
  },
  makeSpecial: function (special_car_data) {
    // console.log special_car_data
